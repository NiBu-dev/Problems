...
const findDistictModulos = (numbers) => {
	const numbersofModulos = new Set();
	numbers.forEach(number => numbersofModulos.add((+number) % 42))
	return numbersofModulos.size;
...
rl.on('line', (line) => {
    ...
    lineCounter++;
    if (lineCounter === 10) {
    	console.log(findDistictModulos(numbers));
    	rl.close();    	
    };
});
